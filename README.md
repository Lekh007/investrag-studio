...
InvestRAG Studio is a standalone, local-first document intelligence and RAG evaluation project for investment research.
...
Every query reports how its answer was produced, because the degraded paths are
otherwise indistinguishable from a working one — the extractive fallback emits `[S1]`
labels and therefore satisfies citation validation exactly like a real completion.
`trace.generation_mode` is one of:

| Mode | Meaning |
|------|---------|
| `model` | the local model wrote the answer and its citations resolve |
| `extractive-fallback` | Ollama was unreachable or returned nothing; the text is verbatim retrieved evidence and is labelled as such in the answer body, a validator message and the UI badge |
| `abstained` | retrieval found no supporting evidence, so **the model was never called** |

The `abstained` mode is load-bearing. Asked a question the corpus does not cover,
a local model will answer from parametric memory and invent both a figure and an `[S1]`
label (measured 2026-08-29 with llama3.1:8b: a fabricated gold price). Abstention is
therefore decided by retrieval, never by the model.
...
For the real local embedding model, omit the environment override. The first query will download/load `BAAI/bge-m3`; keep the laptop plugged in and allow sufficient disk space. Verified 2026-08-29 on this machine: cold load 94 s, 1024 dimensions, unit-normalised vectors. `hash` mode is for offline tests only and is reported as a fallback everywhere it is used.

## Run the frontend

```powershell
cd path\to\investrag-studio\frontend
npm install
npm run dev
```
...
### Frontend PDF renderer (closes design §26's open decision)

**`react-pdf` 10.5.0 (MIT)**, over driving **`pdfjs-dist` 5.4.296 (Apache-2.0)** directly.

Both are permissively licensed, both ship as plain JavaScript with no native build step,
and neither has anything equivalent to `trust_remote_code` risk. react-pdf is a thin
React wrapper *around* pdfjs-dist, so the heavy dependency is identical either way; what
it buys is the genuinely fiddly part — canvas lifecycle across page changes, the text
layer, cancellation of in-flight render tasks on unmount, and a document cache.

Measured, not estimated. Imported statically it put the whole application in one
1,052 kB chunk, so it is `React.lazy`-loaded instead:

| Chunk | Size | Gzip |
|---|---|---|
| initial application | 632 kB | 186 kB |
| PDF renderer (on first document open) | 420 kB | 124 kB |
| pdf.js worker | 1,046 kB | — |

Four of the five views never render a PDF, so they never pay for it. The worker is
served from `node_modules` through Vite, never a CDN, so the localhost-only trust
boundary in design §18 still holds.

Bounding boxes are drawn from real parser output: `CanonicalElement.bbox` is in PDF
points with a TOPLEFT origin whichever parser produced it, `GET
/source-versions/{id}/pages` reports the page box in the same units, and a pdf.js
viewport at scale *S* is exactly `points × S` pixels — so the mapping is one multiply
with no per-parser special-casing in the UI. `chunking.py` carries *every* contributing
element's box into `metadata.source_locations`, not only the first, because a chunk
usually spans several layout blocks.
